...
def run(path):
    ...
    for dir_level in ['inner', 'outer']:
        for file in os.listdir(os.path.join(dir_tsv, dir_level)):
            if '_train' in file:
                ...
                # Load the dataset
                X_data, y_data = load_svmlight_file(os.path.join(dir_tsv, dir_level, file))

                X_data = X_data[:X_data.shape[0] - 1, :]
                y_data = y_data[:len(y_data) - 1]

                with open(os.path.join(dir_arff, dir_level, file.replace('.txt', '.arff')), 'w', encoding='utf-8') as f:
