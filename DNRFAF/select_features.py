...
def run(path):
    ...
    for dir_level in ['inner', 'outer']:
        for file in os.listdir(os.path.join(dir_data, dir_level)):
            print('{0} Selection features for {1}'.format(datetime.now().strftime("%Y-%m-%d_%H:%M:%S"), file))

            data = loader.load_file(os.path.join(dir_data, dir_level, file))
            data.class_is_last()
            ...
            with open(os.path.join(dir_feature_indexes, dir_level, file.replace('_vectors.arff', '_feature_indexes.txt')), 'w', encoding='utf-8') as f:
