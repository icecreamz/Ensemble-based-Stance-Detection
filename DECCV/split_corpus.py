...
def run(path):
    if len(path) == 0:
        dir_src = os.path.join('..', 'data', 'vectors', 'word2vec')
        dir_src_feature_indexes = os.path.join('..', 'DSRFE', 'result_ensemble_fs', 'relevant_feature_indexes')
    else:
        dir_src = os.path.join('data', 'vectors', 'word2vec')
        dir_src_feature_indexes = os.path.join('DSRFE', 'result_ensemble_fs', 'relevant_feature_indexes')
    dir_inner = os.path.join(path, 'vectors_tsv', 'inner')
    dir_outer = os.path.join(path, 'vectors_tsv', 'outer')
    dir_dest_feature_indexes = os.path.join(path, 'feature_indexes')
    ...
    for file in os.listdir(dir_src_feature_indexes):
        copyfile(os.path.join(dir_src_feature_indexes, file), os.path.join(dir_dest_feature_indexes, file))
